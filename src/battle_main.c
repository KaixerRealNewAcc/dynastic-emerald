                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            if (partyData[monIndex].isShiny)
            {
                ...
                u8 level1 = GetHighestLevelInPlayerParty() - 1;
                u8 level2 = GetHighestLevelInPlayerParty() - 2;
                u8 level3 = GetHighestLevelInPlayerParty() - 3;
                u8 level4 = GetHighestLevelInPlayerParty() - 4;
            ...
            if(partyData[i].lvl == PLAYER_MAX)
                CreateMon(&party[i], partyData[monIndex].species, level, 31, TRUE, personalityValue, otIdType, fixedOtId);
            else if(partyData[i].lvl == ONE_BELOW_PLAYER_MAX)
                CreateMon(&party[i], partyData[monIndex].species, level1, 31, TRUE, personalityValue, otIdType, fixedOtId);
            else if (partyData[i].lvl == TWO_BELOW_PLAYER_MAX)
                CreateMon(&party[i], partyData[monIndex].species, level2, 31, TRUE, personalityValue, otIdType, fixedOtId);
            else if (partyData[i].lvl == THREE_BELOW_PLAYER_MAX)
                CreateMon(&party[i], partyData[monIndex].species, level3, 31, TRUE, personalityValue, otIdType, fixedOtId);
            else if (partyData[i].lvl == FOUR_BELOW_PLAYER_MAX)
                CreateMon(&party[i], partyData[monIndex].species, level4, 31, TRUE, personalityValue, otIdType, fixedOtId);
            else
                CreateMon(&party[i], partyData[monIndex].species, partyData[monIndex].lvl, 31, TRUE, personalityValue, otIdType, fixedOtId);
...
u32 GetBattlerTotalSpeedStat(enum BattlerId battler, enum Ability ability, enum HoldEffect holdEffect)
{
    ...
    if (HasWeatherEffect())
    {
        if ((ability == ABILITY_SWIFT_SWIM || innateAbility == ABILITY_SWIFT_SWIM) && !hasAbilityOrInnate(battler, ABILITY_WEATHER_CONTROL) && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_RAIN)
            ...
            speed *= 2;
        else if ((ability == ABILITY_SLUSH_RUSH || innateAbility ==  ABILITY_SLUSH_RUSH)  && !hasAbilityOrInnate(battler, ABILITY_WEATHER_CONTROL) && (gBattleWeather & B_WEATHER_ICY_ANY))
            speed *= 2;
    }
    ...
    if (gBattleMons[battler].status1 & STATUS1_PARALYSIS && (ability != ABILITY_QUICK_FEET || innateAbility !=  ABILITY_QUICK_FEET))
        speed /= GetGenConfig(CONFIG_PARALYSIS_SPEED) >= GEN_7 ? 2 : 4;
...
s32 GetBattleMovePriority(enum BattlerId battler, enum Ability ability, enum Move move)
{
    ...
    if (gProtectStructs[battler].quash)
    {
        ...
        && (GetGenConfig(CONFIG_GALE_WINGS) < GEN_7 || IsBattlerAtMaxHp(battler))
