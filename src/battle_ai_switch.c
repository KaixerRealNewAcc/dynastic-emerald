...
static u32 GetSwitchinHitsToKO(s32 damageTaken, enum BattlerId battler, const struct IncomingHealInfo *healInfo, u32 originalHp)
{
    ...
    while (currentHP > 0)
    {
        ...
        if (currentHP > 0 && !HasAbilityOrInnateFast(battler, ABILITY_KLUTZ) && usedSingleUseHealingItem == FALSE
...
static bool32 AI_CanSwitchinAbilityTrapOpponent(enum Ability ability, enum BattlerId opposingBattler)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_TRAINER && CountUsablePartyMons(opposingBattler) == 0)
        return FALSE;
    else if (hasAbilityOrInnate(opposingBattler, ABILITY_SHADOW_TAG))
    {
        if (B_SHADOW_TAG_ESCAPE >= GEN_4 && hasAbilityOrInnateAI(opposingBattler, ABILITY_SHADOW_TAG)) // Check if ability exists in species
            return FALSE;
        else
            return TRUE;
    }
    else if (hasAbilityOrInnate(opposingBattler, ABILITY_ARENA_TRAP) && IsBattlerGrounded(opposingBattler, gAiLogicData->abilities[opposingBattler], gAiLogicData->holdEffects[opposingBattler]))
        return TRUE;
    else if (hasAbilityOrInnate(opposingBattler, ABILITY_MAGNET_PULL) && IS_BATTLER_OF_TYPE(opposingBattler, TYPE_STEEL))
        return TRUE;
    else
        return FALSE;
}
