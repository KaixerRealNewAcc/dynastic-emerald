...
enum Tags{
    TAG_HELD_ITEM              = 55120,
    TAG_HOVER_CURSOR           = 55121,
    TAG_HOVER_ITEM             = 55122,
    TAG_HELD_ITEM_ICON_BASE    = 55123,
    TAG_SELECT_FRAME           = 55130,
    TAG_MON_SHADOW             = 55140,
    TAG_SWITCH_ITEM_1          = 55141,
    TAG_SWITCH_ITEM_2          = 55142,
    TAG_MESSAGE_WINDOW         = 55150,
    TAG_MULTIUSE_WINDOW        = 55151,
    TAG_MOVE_TYPES             = 55160,
};
...
static const u32 sPartyMenuBg_Gfx[]            = INCBIN_U32("graphics/party_menu/bg.4bpp.smol");
static const u16 sPartyMenuBg_Pal[]            = INCBIN_U16("graphics/party_menu/bg.gbapal");
static const u32 sPartyMenuBg_Main_Tilemap[]   = INCBIN_U32("graphics/party_menu/bg_main.bin.smolTM");
...
static u8 CreateItemIconSprite(struct Pokemon *mon, s16 *state, u16 item);
static void DestroyItemIconSprite();
static void UpdatePartyMonItemIconSprite(u8);
static void ShowOrHideItemIconSprite(u16 item);
...
static void DisplayPartyPokemonDataForChooseFour(u8);
...
static void Task_ValidateChosenFourthParty(u8 taskId);
...
static void BlitBitmapToPartyWindow_MonsColumn(u8, u8, u8, u8, u8, bool8);
...
static void InitPartyMenu(u8 menuType, u8 layout, u8 partyAction, bool8 keepCursorPos, u8 messageId, TaskFunc task, MainCallback callback)
{
    ...
    else
    {
        ...
        else if (menuType == PARTY_MENU_TYPE_CHOOSE_FOUR)
            sPartyMenuInternal->chooseFour = TRUE;
        else
        {
            sPartyMenuInternal->chooseHalf = FALSE;
            sPartyMenuInternal->chooseFour = FALSE;
        }
...
static bool8 ShowPartyMenu(void)
{
    ...
    case 16:
        if (gPartyMenu.menuType != PARTY_MENU_TYPE_MULTI_SHOWCASE
            ...
            && GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_HELD_ITEM) != ITEM_NONE)
        {
            sItemIconSpriteId = CreateItemIconSprite(&gPlayerParty[gPartyMenu.slotId], &sPartyMenuInternal->switchCounter, item);
...
static bool8 DecompressGraphics(void)
{
    ...
    case 2:
        ...
        break;
    case 3:
        LoadPalette(sPartyMenuBg_Pal, BG_PLTT_ID(0), 11 * PLTT_SIZE_4BPP);
...
static void LoadPartyMenuBoxes(u8 layout)
{
    ...
    if (layout == PARTY_LAYOUT_MULTI_SHOWCASE)
        sPartyMenuBoxes[3].infoRects = &sPartyBoxInfoRects[PARTY_BOX_RIGHT_COLUMN];
    else if (layout != PARTY_LAYOUT_SINGLE)
        sPartyMenuBoxes[1].infoRects = &sPartyBoxInfoRects[PARTY_BOX_RIGHT_COLUMN];
}
...
static void RenderPartyMenuBox(u8 slot)
{
    ...
    else if (gPlayerPartyCount != 0)
    {
        ...
        else
        {
            ...
            else if (gPartyMenu.menuType == PARTY_MENU_TYPE_CHOOSE_FOUR)
                DisplayPartyPokemonDataForChooseFour(slot);
...
static void DisplayPartyPokemonDataForChooseFour(u8 slot)
{
    u8 i;
    struct Pokemon *mon = &gPlayerParty[slot];
    u8 *order = gSelectedOrderFromParty;
    ...
    else
    {
        ...
        DisplayPartyPokemonDescriptionData(slot, PARTYBOX_DESC_ABLE_3); // maybe change to PARTYBOX_DESC_ABLE_4?
    }
}
...
static void UpdatePartyMonItemIconSprite(u8 slotId)
{
    s16 state;
    u8 spriteId;
    struct Pokemon *mon = &gPlayerParty[gPartyMenu.slotId]; 
    ...
    if (gPartyMenu.menuType != PARTY_MENU_TYPE_MULTI_SHOWCASE
        ...
        && GetMonData(&gPlayerParty[slotId], MON_DATA_HELD_ITEM) != ITEM_NONE)
    {
        DestroyItemIconSprite();
        state = 0;
        do
        {
            spriteId = CreateItemIconSprite(&gPlayerParty[slotId], &state, item);
        } while (spriteId == 0xFF);
        sItemIconSpriteId = spriteId;
    }
    else
    {
        DestroyItemIconSprite();
...
static void UpdateCurrentPartySelection(s8 *slotPtr, s8 movementDir)
{
    ...
    if (*slotPtr != newSlotId)
    {
        ...
        UpdatePartyMonItemIconSprite(*slotPtr);
    }
}
...
static void BlitBitmapToPartyWindow_RightColumn(u8 windowId, u8 x, u8 y, u8 width, u8 height, bool8 hideHP)
{
    ...
    //if (hideHP == FALSE)
        //BlitBitmapToPartyWindow(windowId, sSlotTilemap_Wide, 16, x, y, width, height); // sSlotTilemap_Wide
    //else
       // BlitBitmapToPartyWindow(windowId, sSlotTilemap_WideNoHP, 16, x, y, width, height); // sSlotTilemap_WideNoHP
}

static void BlitBitmapToPartyWindow_MonsColumn(u8 windowId, u8 x, u8 y, u8 width, u8 height, bool8 hideHP)
{
    if (width == 0 && height == 0)
    {
        width = 14;
    ...
    BlitBitmapToPartyWindow(windowId, sSlotTilemap_Wide, 10, x, y, width, height);
}


static void DrawEmptySlot(u8 windowId)
{
    BlitBitmapToPartyWindow(windowId, sSlotTilemap_WideEmpty, 10, 0, 0, 16, 4);
}
...
static void DisplayPartyPokemonGender(u8 gender, u16 species, u8 *nickname, struct PartyMenuBox *menuBox)
{
    ...
    {
    case MON_MALE:
        LoadPalette(GetPartyMenuPalBufferPtr(sGenderMalePalIds[0]), sGenderPalOffsets[0], PLTT_SIZEOF(1));
        LoadPalette(GetPartyMenuPalBufferPtr(sGenderMalePalIds[1]), sGenderPalOffsets[1], PLTT_SIZEOF(1));
        DisplayPartyPokemonBarDetail(menuBox->windowId, gText_MaleSymbol, 2, &menuBox->infoRects->dimensions[8]);
        break;
    case MON_FEMALE:
        LoadPalette(GetPartyMenuPalBufferPtr(sGenderFemalePalIds[0]), sGenderPalOffsets[0], PLTT_SIZEOF(1));
        LoadPalette(GetPartyMenuPalBufferPtr(sGenderFemalePalIds[1]), sGenderPalOffsets[1], PLTT_SIZEOF(1));
...
static void DisplayPartyPokemonMaxHP(u16 maxhp, struct PartyMenuBox *menuBox)
{
    ...
    ConvertIntToDecimalStringN(gStringVar2, maxhp, STR_CONV_MODE_LEFT_ALIGN, fourDigits ? 4 : 3);
...
static void DisplayPartyPokemonHPBar(u16 hp, u16 maxhp, struct PartyMenuBox *menuBox)
{
    ...
    case HP_BAR_GREEN:
    case HP_BAR_FULL:
        LoadPalette(GetPartyMenuPalBufferPtr(sHPBarGreenPalIds[0]), sHPBarPalOffsets[0], PLTT_SIZEOF(1));
        LoadPalette(GetPartyMenuPalBufferPtr(sHPBarGreenPalIds[1]), sHPBarPalOffsets[1], PLTT_SIZEOF(1));
        break;
    case HP_BAR_YELLOW:
        LoadPalette(GetPartyMenuPalBufferPtr(sHPBarYellowPalIds[0]), sHPBarPalOffsets[0], PLTT_SIZEOF(1));
        LoadPalette(GetPartyMenuPalBufferPtr(sHPBarYellowPalIds[1]), sHPBarPalOffsets[1], PLTT_SIZEOF(1));
        break;
    default:
        LoadPalette(GetPartyMenuPalBufferPtr(sHPBarRedPalIds[0]), sHPBarPalOffsets[0], PLTT_SIZEOF(1));
        LoadPalette(GetPartyMenuPalBufferPtr(sHPBarRedPalIds[1]), sHPBarPalOffsets[1], PLTT_SIZEOF(1));
        break;
    }
...
void DisplayPartyMenuStdMessage(u32 stringId)
{
    ...
    if (stringId != PARTY_MSG_NONE)
    {
        ...
        if (stringId == PARTY_MSG_CHOOSE_MON)
        {
            ...
            else if (sPartyMenuInternal->chooseFour)
                stringId = PARTY_MSG_CHOOSE_MON_AND_CONFIRM;
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (IsNuzlockeModeActive() && GetMonData(&gPlayerParty[0], MON_DATA_DEAD))
    {
...
static u8 GetPartyMenuActionsType(struct Pokemon *mon)
{
    ...
    case PARTY_MENU_TYPE_CHOOSE_HALF:
        ...
        break;
    case PARTY_MENU_TYPE_CHOOSE_FOUR:
        switch (GetPartySlotEntryStatus(gPartyMenu.slotId))
        {
        default: // Not eligible
            actionType = ACTIONS_SUMMARY_ONLY;
...
static void FinishTwoMonAction(u8 taskId)
{
    ...
    UpdatePartyMonItemIconSprite(gPartyMenu.slotId);
...
static u8 CreateItemIconSprite(struct Pokemon *mon, s16 *state, u16 item)
{
    sItemIconSpriteId = AddItemIconSprite(TAG_HELD_ITEM, TAG_HELD_ITEM, item);

    if (sItemIconSpriteId != MAX_SPRITES)
    {
        //Reload the Tiles and Palette cause otherwise you get some weird visual palette bug.
        FreeSpriteTilesByTag(TAG_HELD_ITEM);
        FreeSpritePaletteByTag(TAG_HELD_ITEM);

        gSprites[sItemIconSpriteId].callback = SpriteCallbackDummy;
        gSprites[sItemIconSpriteId].oam.priority = 2;
        gSprites[sItemIconSpriteId].x = 48;
        gSprites[sItemIconSpriteId].y = 128;
    }

    return sItemIconSpriteId;
}
...
static u8 CreateMonSprite(struct Pokemon *mon, bool32 isShadow)
{
    u16 species = GetMonData(mon, MON_DATA_SPECIES_OR_EGG);
    u8 shadowPalette = 0;
    u8 spriteId = CreateSprite(&gMultiuseSpriteTemplate, 44, 66,  5);
...
static void RunMonAnimTimer(void)
{
    ...
    if (sMonAnimTimer > 300 && sMonSpriteId != SPRITE_NONE) // time to re-run the anim
    {
...
void ItemUseCB_Medicine(u8 taskId, TaskFunc task)
{
    ...
    if (cannotUse != FALSE)
    {
        gPartyMenuUseExitCallback = FALSE;
        PlaySE(SE_SELECT);
        if (IsNuzlockeModeActive() && canHeal && GetMonData(mon, MON_DATA_HP) != 0)
            DisplayPartyMenuMessage(gText_WontHaveEffect, TRUE);
        else
            DisplayPartyMenuMessage(gText_WontHaveEffect, TRUE);
        ScheduleBgCopyTilemapToVram(2);
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    else
    {
        ...
        if (sFinalLevel > sInitialLevel)
        {
            ...
            else if (holdEffectParam == 10) // Rare Candy
            {
                ConvertIntToDecimalStringN(gStringVar2, sFinalLevel, STR_CONV_MODE_LEFT_ALIGN, 3);
                StringExpandPlaceholders(gStringVar4, gText_PkmnElevatedToLvVar2);  
            }
...
#define MOSAIC_ANIM_SPRITE_DURATION 25
...
static u8 LoadAndApplyMosaicToMonSprite(struct Pokemon *mon, bool32 isShadow)
{
    ...
    if (spriteId != MAX_SPRITES)
    {
        ...
        gSprites[spriteId].data[5] = MOSAIC_ANIM_SPRITE_DURATION;
...
void InitChooseFourPokemonForBattle(u8 unused)
{
    ClearSelectedPartyOrder();
    InitPartyMenu(PARTY_MENU_TYPE_CHOOSE_FOUR, PARTY_LAYOUT_SINGLE, PARTY_ACTION_CHOOSE_MON, FALSE, PARTY_MSG_CHOOSE_MON, Task_HandleChooseMonInput, gMain.savedCallback);
    gPartyMenu.task = Task_ValidateChosenFourthParty;
}
...
static bool8 GetBattleEntryEligibility(struct Pokemon *mon)
{
    ...
    if (GetMonData(mon, MON_DATA_IS_EGG)
        || GetMonData(mon, MON_DATA_LEVEL) > GetCurrentLevelCap())//GetBattleEntryLevelCap()
    {
        return FALSE;
    }
...
static u8 CheckBattleEntriesAndGetMessage(void)
{
    ...
    if (facility == FACILITY_UNION_ROOM || facility == FACILITY_MULTI_OR_EREADER || facility == FACILITY_MULTI_FOUR)
        return 0xFF;
...
static void Task_ContinueChoosingFourthParty(u8 taskId)
{
    if ((JOY_NEW(A_BUTTON)) || (JOY_NEW(B_BUTTON)))
    {
        PlaySE(SE_SELECT);
...
static void Task_ValidateChosenFourthParty(u8 taskId)
{
    ...
    if (msgId != 0xFF)
    {
        ...
        gTasks[taskId].func = Task_ContinueChoosingFourthParty;
    }
    else
    {
        PlaySE(SE_SELECT);
...
static const u8 *GetFacilityCancelString(void)
{
    ...
    else if (facilityNum == FACILITY_MULTI_FOUR)
        return gText_CancelBattle;
