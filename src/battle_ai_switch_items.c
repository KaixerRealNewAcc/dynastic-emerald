...
static bool32 ShouldSwitchIfHasBadOdds(u32 battler)
{
    ...
    if (maxDamageTaken > gBattleMons[battler].hp
        ...
        || (!IsMoldBreakerTypeAbility(opposingBattler, gAiLogicData->abilities[opposingBattler]) 
        && GetConfig(CONFIG_STURDY) >= GEN_5 && hasAbilityOrInnateAI(battler, ABILITY_STURDY))))
    {
        getsOneShot = TRUE;
    }
