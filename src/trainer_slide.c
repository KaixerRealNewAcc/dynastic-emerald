...
static const u8* const sTrainerSlides[DIFFICULTY_COUNT][TRAINER_PARTNER(PARTNER_COUNT)][TRAINER_SLIDE_COUNT] =
{
    [DIFFICULTY_NORMAL] =
    {
        [TRAINER_ROXANNE_1] =
        {
            [TRAINER_SLIDE_ENEMY_LANDS_FIRST_CRITICAL_HIT] = COMPOUND_STRING("Aha a critical hit, now i got you!{PAUSE_UNTIL_PRESS}"),
            [TRAINER_SLIDE_LAST_LOW_HP] = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX}'s health might be low,\nbut we ain't giving up just yet!{PAUSE_UNTIL_PRESS}"),
        },
    },
};
