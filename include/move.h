...
static inline bool32 IsStrikingMove(enum Move moveId)
{
    return gMovesInfo[SanitizeMoveId(moveId)].strikingMove;
}
...
static inline bool32 IsFieldMove(enum Move moveId)
{
    return gMovesInfo[SanitizeMoveId(moveId)].fieldMove;
}

static inline bool32 IsEruptiveMove(enum Move moveId)
{
